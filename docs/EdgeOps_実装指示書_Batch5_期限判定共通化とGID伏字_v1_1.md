...
- 根拠判定：**EO-DEC-0187**（Batch5）／**EO-DEC-0196**（論点1 GID伏字・論点7 期限条件）／**EO-DEC-0198**（条件C・D）／**EO-DEC-0199**（取得経路の統一）／**EO-DEC-0201**（列追加範囲の縮小・`joinGroup` 例外）／**EO-DEC-0202**（`restoreSession` 例外・3条件・undefined の扱い）
...
**★RPC本体を確認済み：`join_group_with_member` の `jsonb_build_object` は5経路すべてで `expires_at`・`facility_id`・`archived_at` を返していない。** よってRPC戻り値からの手組みでは列を補えない。不可触RPCは変更しないため（EO-DEC-0201 論点2-b）、**`group_sessions` を実際に取得する方式（案ア-1）を採る。**
...
- 既存の手組みオブジェクト（`const` 宣言や `currentGroup = {...}` のプロパティ行）を**削除・改名しない**。使用されなくなる場合も定義は残し、直上に `// [EO-DEC-0201] 本オブジェクトは使用しない。既存行を削除しない条件のため定義のみ残す` のコメントを追加する
...
- **`currentGroupObj` の定義（L2068〜L2074）を削除・改名しない。** 直上に `// [EO-DEC-0201] already_approved では group_sessions を select('*') で取り直すため本オブジェクトは使用しない。既存行は削除しない` のコメントを追加する
...
### 未取得（undefined）の扱い（EO-DEC-0202 論点3）
...
**★3経路とも同一の `isExpiryManaged()` を呼ぶこと。適用条件を各所に書き写さない**（EO-DEC-0202）。
...
- 直上のコメント `// ST版(SL-)の期限切れチェック` は `// [EO-DEC-0202] 1年期限管理の適用対象かを共通関数で判定する` に**書き換えてよい**
...
| `gidMasked` が `undefined`（未取得） | `console.warn` を出したうえで**実値**を返す（現場を止めないためのフェイルセーフ。EO-DEC-0201 により黙って `false` として扱わない） |

**★伏字の判定を各表示箇所に書かない。必ずこの関数を通す**（EO-DEC-0201）。
...
**`pending-group-id`（L2097 / L2108）は伏字にしない。** 本人が直前に入力したGIDの確認表示であり、伏せると自分がどのコードで申請したか分からなくなる（EO-DEC-0201 論点3）。
...
EO-DEC-0199 は当初、列指定SELECT4箇所にも `facility_id`・`archived_at` を追加するとしていた。**EO-DEC-0201 でこれを取り消した。** 理由は実コード追跡の結果、次が確認できたためである。
...
| # | 確認項目 | 期待 |
|---|---|---|
| 16 | 上記A〜Cの全操作中、`[EO-DEC-0202] isExpiryManaged` の警告 | **1件も出ない**（出たら取得経路の漏れ＝要修正） |
